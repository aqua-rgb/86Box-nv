 ...
 *
 *          nv4: Methods shared across multiple classes
 *
 *
 *
...
void nv4_generic_method(uint32_t param, uint32_t method_id, nv4_ramin_context_t context, nv4_grobj_t grobj)
{
    switch (method_id)
    {
        ...
        case NV4_ROOT_HI_IM_OBJECT_MCOBJECTYFACE:
        ...
        case NV4_SET_NOTIFY:
            ...
            {
                nv_log("Executed method NV4_SET_NOTIFY with nv4->pgraph.notify_pending already set. param=0x%08x, method=0x%04x, grobj=0x%08x 0x%08x 0x%08x 0x%08x\n");
...
void nv4_notify_if_needed(uint32_t name, uint32_t method_id, nv4_ramin_context_t context, nv4_grobj_t grobj)
{
    ...
    uint32_t current_notification_object = nv4->pgraph.notifier;
    uint32_t notification_type = ((current_notification_object >> NV4_PGRAPH_NOTIFY_REQUEST_TYPE) & 0x07);

    // check for a software method (0 = hardware, 1 = software)
    if (notification_type != 0)
    {  
        nv_log("Software Notification, firing interrupt");
        nv4_pgraph_interrupt_valid(NV4_PGRAPH_INTR_0_SOFTWARE_NOTIFY);
        //return;
    }
        
    // set up the NvNotification structure
    nv4_notification_t notify = {0}; 
    notify.nanoseconds = nv4->ptimer.time;
    notify.status = NV4_NOTIFICATION_STATUS_DONE_OK; // it should be fine to just signal that it's ok
    ...
    uint32_t notify_obj_info  = nv4_ramin_read32(notify_obj_base, nv4);
    uint32_t notify_obj_limit = nv4_ramin_read32(notify_obj_base + 0x04, nv4);
    uint32_t notify_obj_page  = nv4_ramin_read32(notify_obj_base + 0x08, nv4);

    /* extract some important information*/
    uint32_t info_adjust = notify_obj_info & 0xFFF;
    bool info_pt_present = (notify_obj_info >> NV4_NOTIFICATION_PT_PRESENT) & 0x01;
    uint8_t info_notification_target = (notify_obj_info >> NV4_NOTIFICATION_TARGET) & 0x03;

    /* paging information */
    bool page_is_present = notify_obj_page & 0x01;
    bool page_is_readwrite = (notify_obj_page >> NV4_NOTIFICATION_PAGE_ACCESS);
    ...
    {
        case NV4_NOTIFICATION_TARGET_NVM: 
            nv_log_verbose_only("Notification Target: VRAM\n");
            break;
        case NV4_NOTIFICATION_TARGET_CART: 
            nv_log_verbose_only("VERY BAD WARNING: Notification detected with Notification Target: Cartridge. THIS SHOULD NEVER HAPPEN!!!!!\n");
            break;
        case NV4_NOTIFICATION_TARGET_PCI: 
            (nv4->nvbase.bus_generation == nv_bus_pci) ? nv_log_verbose_only("Notification Target: PCI Bus\n") : nv_log_verbose_only("Notification Target: PCI Bus (On AGP card?)\n");
            break;
        case NV4_NOTIFICATION_TARGET_AGP: 
            (nv4->nvbase.bus_generation == nv_bus_agp_1x
                || nv4->nvbase.bus_generation == nv_bus_agp_2x) ? nv_log_verbose_only("Notification Target: AGP Bus\n") : nv_log_verbose_only("Notification Target: AGP Bus (On PCI card?)\n");
            break;
    }
    ...
    else
    {
        ...
        final_address = nv4_ramin_read32(notify_obj_base + (0x10 * pte_num) + 8, nv4);
    ...
    {
        case NV4_NOTIFICATION_TARGET_NVM:
            svga_writel_linear(final_address, (notify.nanoseconds & 0xFFFFFFFF), &nv4->nvbase.svga);
            svga_writel_linear(final_address + 4, (notify.nanoseconds >> 32), &nv4->nvbase.svga);
            svga_writel_linear(final_address + 8, notify.info32, &nv4->nvbase.svga);
            svga_writel_linear(final_address + 0x0C, (notify.info16 | notify.status), &nv4->nvbase.svga);
            break;
        case NV4_NOTIFICATION_TARGET_PCI:
        case NV4_NOTIFICATION_TARGET_AGP:
            dma_bm_write(final_address, (uint8_t*)&notify, sizeof(nv4_notification_t), 4);
            break;
    }
