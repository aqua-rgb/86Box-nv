...
void nv4_render_8bpp(nv4_position_16_t position, nv4_size_16_t size, nv4_grobj_t grobj);
void nv4_render_15bpp(nv4_position_16_t position, nv4_size_16_t size, nv4_grobj_t grobj);
void nv4_render_16bpp(nv4_position_16_t position, nv4_size_16_t size, nv4_grobj_t grobj);
void nv4_render_32bpp(nv4_position_16_t position, nv4_size_16_t size, nv4_grobj_t grobj);

/* Expand a colour.
   NOTE: THE GPU INTERNALLY OPERATES ON RGB10!!!!!!!!!!!
*/
nv4_color_expanded_t nv4_render_expand_color(uint32_t color, nv4_grobj_t grobj)
{
    ...
    switch (format)
    {
        ...
        // 555
        case nv4_pgraph_pixel_format_r5g5b5:
        ...
        case nv4_pgraph_pixel_format_r8g8b8:
            if (alpha_enabled)
            ...
            break;
        case nv4_pgraph_pixel_format_r10g10b10:
        ...
        case nv4_pgraph_pixel_format_y16:
            ...
            break;
        default:
            warning("nv4_render_expand_color unknown format %d", format);
            break;
...
uint32_t nv4_render_downconvert_color(nv4_grobj_t grobj, nv4_color_expanded_t color)
{
    uint8_t format = (grobj.grobj_0 & 0x07);
    bool alpha_enabled = (grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_ALPHA) & 0x01;
    ...
    switch (format)
    {
        ...
        case nv4_pgraph_pixel_format_r10g10b10:
            ...
            break;
        case nv4_pgraph_pixel_format_y8:
            warning("nv4_render_downconvert: Y8 not implemented");
            break;
        case nv4_pgraph_pixel_format_y16:
            warning("nv4_render_downconvert: Y16 not implemented");
            break;
        default:
            warning("nv4_render_downconvert_color unknown format %d", format);
            break;
...
{
    bool chroma_enabled = ((grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_CHROMA_KEY) & 0x01);

    if (!chroma_enabled)
        return true;
    
    bool alpha = ((nv4->pgraph.chroma_key >> 31) & 0x01);

    if (!alpha)
        return true;

    /* this is dumb but i'm lazy, if it kills perf, fix it later - we need to do some format shuffling */
    nv4_grobj_t grobj_fake = {0};
    grobj_fake.grobj_0 = 0x02; /* we don't care about any other bits */

    nv4_color_expanded_t chroma_expanded = nv4_render_expand_color(nv4->pgraph.chroma_key, grobj_fake);
   
    uint32_t chroma_downconverted = nv4_render_downconvert_color(grobj, chroma_expanded);
...
uint32_t nv4_render_set_pattern_color(nv4_color_expanded_t pattern_colour, bool use_color1)
{
    /* reset the colour */
    if (!use_color1)
        nv4->pgraph.pattern_color_0_rgb.r = nv4->pgraph.pattern_color_0_rgb.g = nv4->pgraph.pattern_color_0_rgb.b = 0x00;
    else 
        nv4->pgraph.pattern_color_1_rgb.r = nv4->pgraph.pattern_color_1_rgb.g = nv4->pgraph.pattern_color_1_rgb.b = 0x00;
   
    /* select the right pattern colour, _rgb is already in RGB10 format, so we don't need to do any conversion */

    if (!use_color1)
    {
        nv4->pgraph.pattern_color_0_alpha = (pattern_colour.a) & 0xFF;
        nv4->pgraph.pattern_color_0_rgb.r = pattern_colour.r;
        nv4->pgraph.pattern_color_0_rgb.g = pattern_colour.g;
        nv4->pgraph.pattern_color_0_rgb.b = pattern_colour.b;

    }
    else 
    {
        nv4->pgraph.pattern_color_1_alpha = (pattern_colour.a) & 0xFF;
        nv4->pgraph.pattern_color_1_rgb.r = pattern_colour.r;
        nv4->pgraph.pattern_color_1_rgb.g = pattern_colour.g;
        nv4->pgraph.pattern_color_1_rgb.b = pattern_colour.b;
    }
...
uint32_t nv4_render_get_vram_address(nv4_position_16_t position, nv4_grobj_t grobj)
{
    ...
    uint32_t current_buffer = (grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_SRC_BUFFER) & 0x03; 

    uint32_t destination_buffer = 5; // 5 = just use the source buffer

    // src is hardcoded to 1, dst to 0. Hmm...
    if ((grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_DST_BUFFER0_ENABLED) & 0x01) destination_buffer = 0;
    if ((grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_DST_BUFFER1_ENABLED) & 0x01) destination_buffer = 1;
    if ((grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_DST_BUFFER2_ENABLED) & 0x01) destination_buffer = 2;
    if ((grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_DST_BUFFER3_ENABLED) & 0x01) destination_buffer = 3;

    if (destination_buffer != current_buffer
    && destination_buffer != 5)
        current_buffer = destination_buffer;

    uint32_t framebuffer_bpp = nv4->nvbase.svga.bpp;
    ...
    uint32_t pixel_addr_vram = vram_x + (nv4->pgraph.bpitch[current_buffer] * vram_y) + nv4->pgraph.boffset[current_buffer];

    pixel_addr_vram &= nv4->nvbase.svga.vram_mask;
...
nv4_position_16_t nv4_render_get_dfb_position(uint32_t vram_address)
{
    nv4_position_16_t pos = {0};

    uint32_t pitch = nv4->nvbase.svga.hdisp;
    ...
    //vram_address -= nv4->pgraph.boffset[0];
    ...
    if (pos.y >= nv4->nvbase.svga.monitor->target_buffer->h) pos.y = nv4->nvbase.svga.monitor->target_buffer->h - 1;
...
{ 
    ...
    uint32_t vram_address = nv4_render_get_vram_address(position, grobj);

    return nv4->nvbase.svga.vram[vram_address];
}
...
void nv4_render_write_pixel(nv4_position_16_t position, uint32_t color, nv4_grobj_t grobj)
{
    ...
    #ifdef DEBUG
    uint8_t color_format_object = (grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_COLOR_FORMAT) & 0x07;
    #endif
    bool alpha_enabled = (grobj.grobj_0 >> NV4_PGRAPH_CONTEXT_SWITCH_ALPHA) & 0x01;

    uint32_t framebuffer_bpp = nv4->nvbase.svga.bpp; // maybe y16 too?z

    /* doesn't seem*/
    nv4_color_argb_t color_data = *(nv4_color_argb_t*)&color;

    if (framebuffer_bpp == 32)
        alpha = color_data.a;
    
    int32_t clip_end_x = nv4->pgraph.clip_start.x + nv4->pgraph.clip_size.x;
    int32_t clip_end_y = nv4->pgraph.clip_start.y + nv4->pgraph.clip_size.y;
    
    /* First, check our current buffer. */
    /* Then do the clip. */
    if (position.x < nv4->pgraph.clip_start.x
        || position.y < nv4->pgraph.clip_start.y
    ...
    /* TODO: Plane Mask...*/
    if (!nv4_render_chroma_test(color, grobj))
        return;

    uint32_t pixel_addr_vram = nv4_render_get_vram_address(position, grobj);

    uint32_t rop_src = 0, rop_dst = 0, rop_pattern = 0;
    uint8_t bit = 0x00; 

    /* Get our pattern data, may move to another function */
    switch (nv4->pgraph.pattern.shape)
    {

        /* This logic is from NV1 envytoos docs, but seems to be same on nv4*/
        case NV4_PATTERN_SHAPE_8X8:
            bit = (position.x & 7) | (position.y & 7) << 3;
            break; 
        case NV4_PATTERN_SHAPE_1X64:
            bit = (position.x & 0x3f);
            break;
        case NV4_PATTERN_SHAPE_64X1:
            bit = (position.y & 0x3f);
            break;
    }
    ...
    bool use_color1 = (nv4->pgraph.pattern_bitmap >> bit) & 0x01;

    if (!use_color1)
    {
        if (!nv4->pgraph.pattern_color_0_alpha)
            return; 

        /* This is stupid */
        rop_pattern = nv4_render_downconvert_color(grobj, nv4->pgraph.pattern_color_0_rgb);
    }
    else
    {
        if (!nv4->pgraph.pattern_color_1_alpha)
            return;

        rop_pattern = nv4_render_downconvert_color(grobj, nv4->pgraph.pattern_color_1_rgb);
    }
    ...
    */
    ...
    switch (framebuffer_bpp)
    {
        case 8:
            rop_src = color & 0xFF;
            rop_dst = nv4->nvbase.svga.vram[pixel_addr_vram];
            nv4->nvbase.svga.vram[pixel_addr_vram] = video_rop_gdi_ternary(nv4->pgraph.rop, rop_src, rop_dst, rop_pattern) & 0xFF;

            nv4->nvbase.svga.changedvram[pixel_addr_vram >> 12] = changeframecount;
        ...
        case 16:
            ...
            bool is_16bpp = (nv4->pramdac.general_control >> NV4_PRAMDAC_GENERAL_CONTROL_565_MODE) & 0x01;
            ...
            vram_16[pixel_addr_vram] = video_rop_gdi_ternary(nv4->pgraph.rop, rop_src, rop_dst, rop_pattern) & 0xFFFF;

            nv4->nvbase.svga.changedvram[pixel_addr_vram >> 11] = changeframecount;
        ...
        case 32:
            ...
            vram_32[pixel_addr_vram] = video_rop_gdi_ternary(nv4->pgraph.rop, rop_src, rop_dst, rop_pattern);

            nv4->nvbase.svga.changedvram[pixel_addr_vram >> 10] = changeframecount;

            break;
    }
    
    /* Go write the pixel */
    nv4_size_16_t size = {0};
    size.w = size.h = 1; 
    nv4_render_current_bpp(&nv4->nvbase.svga, position, size, grobj);
}

/* Ensure the correct monitor size */
void nv4_render_ensure_screen_size(void)
{
    bool changed = false; //doesn't check if the res is the same?

    if (nv4->nvbase.svga.hdisp != nv4->nvbase.svga.monitor->mon_xsize)
    {
        changed = true;  
        nv4->nvbase.svga.monitor->mon_xsize = nv4->nvbase.svga.hdisp;
    }

    if (nv4->nvbase.svga.dispend != nv4->nvbase.svga.monitor->mon_ysize)
    {
        changed = true; 
        nv4->nvbase.svga.monitor->mon_ysize = nv4->nvbase.svga.dispend;
    }
    
    if (changed)
    {
        /* set refresh rate - this is just a rough estimation right now. we need it as we only blit what changes */
        nv4->nvbase.refresh_time = 1 / (nv4->nvbase.pixel_clock_frequency / (double)ysize / (double)xsize); // rivatimers count in microseconds
...
void nv4_render_current_bpp_dfb_16(uint32_t address)
{
    ...
    nv4_position_16_t pos = nv4_render_get_dfb_position(address);

    //pos.x >>= 1; 

    uint32_t* p = &nv4->nvbase.svga.monitor->target_buffer->line[pos.y][pos.x];
    uint32_t data = *(uint32_t*)&(nv4->nvbase.svga.vram[address]);
        
    if ((nv4->pramdac.general_control >> NV4_PRAMDAC_GENERAL_CONTROL_565_MODE) & 0x01)
        /* should just "tip over" to the next line */
        *p = nv4->nvbase.svga.conv_16to32(&nv4->nvbase.svga, data & 0xFFFF, 16);
    else
        /* should just "tip over" to the next line */
        *p = nv4->nvbase.svga.conv_16to32(&nv4->nvbase.svga, data & 0xFFFF, 15);
...
void nv4_render_current_bpp_dfb_32(uint32_t address)
{
    ...
    if (nv4->nvbase.svga.bpp == 32)
    {
        uint32_t* p = &nv4->nvbase.svga.monitor->target_buffer->line[pos.y][pos.x];
        *p = data;
    }
    /* Packed format */
    else if (nv4->nvbase.svga.bpp == 15
    || nv4->nvbase.svga.bpp == 16)
    {
        //pos.x >>= 1;
        ...
        *p = nv4->nvbase.svga.conv_16to32(&nv4->nvbase.svga, data & 0xFFFF, nv4->nvbase.svga.bpp);
        *p++;
        *p = nv4->nvbase.svga.conv_16to32(&nv4->nvbase.svga, (data >> 16) & 0xFFFF, nv4->nvbase.svga.bpp);
    }
}


/* Blit to the monitor from GPU, current bpp */
void nv4_render_current_bpp(svga_t *svga, nv4_position_16_t pos, nv4_size_16_t size, nv4_grobj_t grobj)
{
    /* Ensure that we are in the correct mode. Modified SVGA core code */
    nv4_render_ensure_screen_size();

    /* Don't try and draw stuff that is past the buffer, but, leave it in Video RAM */
    //if (nv4->nvbase.last_buffer_address > (((nv4->nvbase.svga.bpp + 1) >> 3) * xsize * ysize))
        //return;
    ...
    switch (nv4->nvbase.svga.bpp)
    {
        case 4:
            ...
            fatal("nv4 - Tried to render 4bpp in NV mode");
            break; 
        case 8:
            nv4_render_8bpp(pos, size, grobj);
            break; 
        case 15:
            nv4_render_15bpp(pos, size, grobj);
            break; 
        case 16:
            nv4_render_16bpp(pos, size, grobj);
            break;
        case 32:
            nv4_render_32bpp(pos, size, grobj);
            break; 
    }
...
void nv4_render_8bpp(nv4_position_16_t pos, nv4_size_16_t size, nv4_grobj_t grobj)
{
...
void nv4_render_15bpp(nv4_position_16_t pos, nv4_size_16_t size, nv4_grobj_t grobj)
{
    if (!nv4)
        return; 
    ...
    p = &nv4->nvbase.svga.monitor->target_buffer->line[pos.y][pos.x];

    for (uint32_t y = 0; y < size.h; y++)
    {
        /* re-set the vram address because we are basically "jumping" halfway across a line here */
        vram_base = nv4_render_get_vram_address(pos, grobj) & nv4->nvbase.svga.vram_display_mask;

        for (uint32_t x = 0; x < size.w; x++)
        {
            p = &nv4->nvbase.svga.monitor->target_buffer->line[pos.y][pos.x];

            data = *(uint32_t*)&nv4->nvbase.svga.vram[vram_base];
...
void nv4_render_16bpp(nv4_position_16_t pos, nv4_size_16_t size, nv4_grobj_t grobj)
{    
    if (!nv4)
        return; 
...
void nv4_render_32bpp(nv4_position_16_t pos, nv4_size_16_t size, nv4_grobj_t grobj)
{
    if (!nv4)
        return; 
