 ...
 *
 *          nv4 User Submission Area (NV_USER, conceptually considered "Cache1 Pusher")
 *
 *
 *
...
uint32_t nv4_user_read(uint32_t address)
{
    ...
    {
        case NV4_SUBCHANNEL_PIO_IS_PFIFO_FREE:
            return nv4_pfifo_cache1_num_free_spaces();
        case NV4_SUBCHANNEL_PIO_ALWAYS_ZERO_START ... NV4_SUBCHANNEL_PIO_ALWAYS_ZERO_END:
            return 0x00;
...
// Although nv4 doesn't have DMA mode unlike NV4 and later, it's conceptually similar to a "pusher" that pushes graphics commands that you write into CACHE1 that are then pulled out.
...
{
    nv4_pfifo_cache1_push(address, value);
