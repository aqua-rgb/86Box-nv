 ...
 *
 *          nv4 PRAMIN - Basically, this is how we know what to render.
...
{
    nv4->pfifo.interrupt_status |= (1 << id);
    nv4_pmc_handle_interrupts(fire_now);
}
...
*/
bool nv4_ramin_arbitrate_read(uint32_t address, uint32_t* value)
{
    if (!nv4) return 0x00;

    uint32_t ramht_size = ((nv4->pfifo.ramht_config >> NV4_PFIFO_CONFIG_RAMHT_SIZE) & 0x03);
    ...
    uint32_t ramht_start = ((nv4->pfifo.ramht_config >> NV4_PFIFO_CONFIG_RAMHT_BASE_ADDRESS) & 0x0F) << 12;  // Must be 0x1000 aligned
    uint32_t ramfc_start = ((nv4->pfifo.ramfc_config >> NV4_PFIFO_CONFIG_RAMFC_BASE_ADDRESS) & 0x7F) << 9;   // Must be 0x200 aligned
    uint32_t ramro_start = ((nv4->pfifo.ramro_config >> NV4_PFIFO_CONFIG_RAMRO_BASE_ADDRESS) & 0x7F) << 9;   // Must be 0x200 aligned

    // Calculate the RAMHT and RAMRO end points.
    // (RAMFC is always 0x1000 bytes on nv4.)
    ...
    switch (ramht_size)
    {
        case NV4_PFIFO_CONFIG_RAMHT_SIZE_4K:
            ramht_end = ramht_start + NV4_RAMIN_RAMHT_SIZE_0;
            break;
        case NV4_PFIFO_CONFIG_RAMHT_SIZE_8K:
            ramht_end = ramht_start + NV4_RAMIN_RAMHT_SIZE_1;
            break;
        case NV4_PFIFO_CONFIG_RAMHT_SIZE_16K:
            ramht_end = ramht_start + NV4_RAMIN_RAMHT_SIZE_2;
            break;
        case NV4_PFIFO_CONFIG_RAMHT_SIZE_32K:
            ramht_end = ramht_start + NV4_RAMIN_RAMHT_SIZE_3;
            break;
    }

    switch (ramro_size)
    {
        case NV4_PFIFO_CONFIG_RAMRO_SIZE_512B:
            ramro_end = ramro_start + NV4_RAMIN_RAMRO_SIZE_0;
            break;
        case NV4_PFIFO_CONFIG_RAMRO_SIZE_8K:
            ramro_end = ramro_start + NV4_RAMIN_RAMRO_SIZE_1;
            break;
    }
    ...
    && address <= ramht_end)
    {
        *value = nv4_ramht_read(address);
        return true;
    }
    else if (address >= ramfc_start 
    && address <= ramfc_end)
    {
        *value = nv4_ramfc_read(address);
        return true;
    }
    else if (address >= ramro_start 
    && address <= ramro_end)
    {
        *value = nv4_ramro_read(address);
        return true;
    }
...
bool nv4_ramin_arbitrate_write(uint32_t address, uint32_t value) 
{
    if (!nv4) return 0x00;
    ...
    && address <= ramht_end)
    {
        nv4_ramht_write(address, value);
        return true;
    }
    else if (address >= ramfc_start 
    && address <= ramfc_end)
    {
        nv4_ramfc_write(address, value);
        return true;
    }
    else if (address >= ramro_start 
    && address <= ramro_end)
    {
        nv4_ramro_write(address, value);
        return true;
    }
...
bool nv4_ramin_find_object(uint32_t name, uint32_t cache_num, uint8_t channel, uint8_t subchannel)
{  
    // TODO: WRITE IT!!!
    ...
    uint8_t ramht_size = (nv4->pfifo.ramht_config >> NV4_PFIFO_CONFIG_RAMHT_SIZE) & 0x03;
    ...
    uint32_t ramht_base = ((nv4->pfifo.ramht_config >> NV4_PFIFO_CONFIG_RAMHT_BASE_ADDRESS) & 0x0F) << NV4_PFIFO_CONFIG_RAMHT_BASE_ADDRESS;
    ...
    // Why does this work?
    uint32_t ramht_cur_address = ramht_base + (nv4_ramht_hash(name, channel) * bucket_entries * 8);
    ...
    nv4_ramin_context_t obj_context_struct = {0};

    for (uint32_t bucket_entry = 0; bucket_entry < bucket_entries; bucket_entry++)
    {
        found_obj_name = nv4_ramin_read32(ramht_cur_address, NULL);
        ramht_cur_address += 0x04;
        uint32_t obj_context = nv4_ramin_read32(ramht_cur_address, NULL);
        ramht_cur_address += 0x04;
        obj_context_struct = *(nv4_ramin_context_t*)&obj_context;
    ...
    if (!found_object)
    {
        if (!cache_num)
        {
            nv4->pfifo.debug_0 |= NV4_PFIFO_CACHE0_ERROR_PENDING;
            nv4->pfifo.cache0_settings.pull0 |= NV4_PFIFO_CACHE0_PULL0_HASH_FAILURE;
            //It turns itself off on failure, the drivers turn it back on
            nv4->pfifo.cache0_settings.pull0 &= ~NV4_PFIFO_CACHE0_PULL0_ENABLED;
        } 
        else 
        {
            nv4->pfifo.debug_0 |= NV4_PFIFO_CACHE1_ERROR_PENDING;
            nv4->pfifo.cache1_settings.pull0 |= NV4_PFIFO_CACHE1_PULL0_HASH_FAILURE;
            //It turns itself off on failure, the drivers turn it back on
            nv4->pfifo.cache1_settings.pull0 &= ~NV4_PFIFO_CACHE1_PULL0_ENABLED;
        }
    ...
    if (obj_context_struct.class_id < NV4_PFIFO_FIRST_VALID_GRAPHICS_OBJECT_ID
    || obj_context_struct.class_id > NV4_PFIFO_LAST_VALID_GRAPHICS_OBJECT_ID)
    {
        fatal("nv4: Invalid graphics object class ID name=0x%04x type=%04x, interpreted by pgraph as: %04x (Contact starfrost)", 
            name, obj_context_struct.class_id, obj_context_struct.class_id & 0x1F);
    }   
    else if (obj_context_struct.channel > (NV4_DMA_CHANNELS - 1))
        fatal("nv4: Super fucked up graphics object. Contact starfrost with the error string: DMA Channel ID=%d, it should be 0-7", obj_context_struct.channel);
    
    // Illegal accesses sent to RAMRO, so ignore here
    // TODO: SEND THESE TO RAMRO!!!!!

    #ifndef RELEASE_BUILD
    nv4_debug_ramin_print_context_info(name, obj_context_struct);
    #endif
    ...
    if (!cache_num)
        is_software = (nv4->pfifo.cache0_settings.context[subchannel] & 0x800000);
    else 
        is_software = (nv4->pfifo.cache1_settings.context[subchannel] & 0x800000);
    ...
    {  
        ...
        {
            nv4->pfifo.cache1_settings.pull0 |= NV4_PFIFO_CACHE1_PULL0_SOFTWARE_METHOD;
            nv4->pfifo.cache1_settings.pull0 &= ~NV4_PFIFO_CACHE1_PULL0_ENABLED;
        }
...
void nv4_debug_ramin_print_context_info(uint32_t name, nv4_ramin_context_t context)
{
    ...
    nv_log_verbose_only("Class ID: =0x%04x (%s)\n", context.class_id & 0x1F, nv4_class_names[context.class_id & 0x1F]);
